...
learun.learuntab = {
    ...
    activeTab: function () {
        ...
        if (!$(this).hasClass('active')) {
            ...
            learun.learuntab.scrollToTab(this);
        }    
    },
    ...
    closeTab: function () {
        ...
        if ($(this).parents('.menuTab').hasClass('active')) {
            ...
            learun.learuntab.scrollToTab($('.menuTab.active'));
        }
        return false;
    },
    addTab: function () {
    ...
    scrollTabRight: function () {
        ...
        var tabOuterWidth = learun.learuntab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
        ...
        if ($(".page-tabs-content").width() < visibleWidth) {
            ...
            scrollVal = learun.learuntab.calSumWidth($(tabElement).prevAll());
            if (scrollVal > 0) {
                $('.page-tabs-content').animate({
    ...
    scrollTabLeft: function () {
        ...
        if ($(".page-tabs-content").width() < visibleWidth) {
            ...
            offsetVal = 0;
            if (learun.learuntab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                    offsetVal += $(tabElement).outerWidth(true);
    ...
    scrollToTab: function (element) {
        var marginLeftVal = learun.learuntab.calSumWidth($(element).prevAll()), marginRightVal = learun.learuntab.calSumWidth($(element).nextAll());
        var tabOuterWidth = learun.learuntab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
        var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
        var scrollVal = 0;
    ...
    init: function () {
        $('.menuItem').on('click', learun.learuntab.addTab);
        $('.menuTabs').on('click', '.menuTab i', learun.learuntab.closeTab);
        $('.menuTabs').on('click', '.menuTab', learun.learuntab.activeTab);
        $('.tabLeft').on('click', learun.learuntab.scrollTabLeft);
        $('.tabRight').on('click', learun.learuntab.scrollTabRight);
        $('.tabReload').on('click', learun.learuntab.refreshTab);
        ...
        $('.tabCloseOther').on('click', learun.learuntab.closeOtherTabs);
        $('.fullscreen').on('click', function () {
            if (!$(this).attr('fullscreen')) {
                $(this).attr('fullscreen', 'true');
                learun.learuntab.requestFullScreen();
            } else {
                $(this).removeAttr('fullscreen')
                learun.learuntab.exitFullscreen();
            }
        });
    }
};
    ...
    loadMenu: function (data) {
        ...
        $.each(data, function (i) {
            ...
            if (row.F_ParentId == "0") {
                ...
                _html += '</a>'
                var childNodes = learun.learunindex.jsonWhere(data, function (v) { return v.F_ParentId == row.F_ModuleId });
                if (childNodes.length > 0) {
                    _html += '<ul class="treeview-menu">';
                    $.each(childNodes, function (i) {
                        var subrow = childNodes[i];
                        var subchildNodes = learun.learunindex.jsonWhere(data, function (v) { return v.F_ParentId == subrow.F_ModuleId });
                        _html += '<li>';
